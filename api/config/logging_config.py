...
def setup_logging(
    ...
    if log_to_file:
        ...
        file_formatter = logging.Formatter(format_string.replace('%(levelname)s', '%(levelname)s').replace('%(service_name)s', '%(service_name)s'))
...
def log_request(logger: logging.Logger, method: str, url: str, status_code: int = None, duration: float = None):
