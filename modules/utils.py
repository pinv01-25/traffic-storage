"""
Utility functions and exceptions for traffic storage service.

This module consolidates validation functions and custom exceptions
...
def validate_traffic_light_id(traffic_light_id: str) -> bool:
    ...
    if not re.match(r"^\d+$", traffic_light_id):
        raise ValueError(
            f"Invalid traffic_light_id format: {traffic_light_id}. Expected only numbers."
        )
    return True
...
def validate_sensor_count(sensors: list) -> bool:
    ...
    if count < settings.MIN_SENSORS_PER_BATCH:
        raise ValueError(
            f"Too few sensors: {count}. Minimum required: {settings.MIN_SENSORS_PER_BATCH}"
        )
    if count > settings.MAX_SENSORS_PER_BATCH:
        raise ValueError(
            f"Too many sensors: {count}. Maximum allowed: {settings.MAX_SENSORS_PER_BATCH}"
        )
    return True
...
def validate_timestamp(timestamp: int) -> bool:
    ...
    if timestamp < min_timestamp or timestamp > max_timestamp:
        raise ValueError(
            f"Timestamp {timestamp} is outside valid range ({min_timestamp}-{max_timestamp})"
        )
