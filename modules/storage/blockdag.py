...
class BlockDAGService:
    ...
    def __init__(self):
        ...
        if not self.rpc_url or self.rpc_url.strip() == "":
            ...
            self.rpc_url = "https://rpc.primordial.bdagscan.com"
    ...
    async def fetch_metadata(self, traffic_light_id: str, timestamp: int, data_type: int) -> str:
        ...
        try:
            logger.debug(
                f"Fetching metadata for traffic_light_id: {traffic_light_id}, timestamp: {timestamp}"
            )
    ...
    def check_connectivity(self) -> bool:
        """
        Check BlockDAG connectivity by attempting to query the node.
