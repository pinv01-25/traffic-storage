...
from modules.storage.blockdag import BlockDAGService
from modules.storage.ipfs import IPFSService
...
class StorageService:
    ...
    async def upload_metadata(self, metadata) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"Starting upload for {metadata.type} data, traffic_light_id: {metadata.traffic_light_id}"
            )

            # Validate data payload
            from modules.utils import validate_data_payload
            ...
            logger.debug(
                f"Metadata stored in BlockDAG for traffic_light_id: {metadata.traffic_light_id}"
            )
    ...
    async def download_metadata(self, request) -> Dict[str, Any]:
        """
        ...
        try:
            logger.info(
                f"Starting download for {request.type} data, traffic_light_id: {request.traffic_light_id}"
            )
    ...
    def _prepare_upload_response(self, metadata, cid: str) -> Dict[str, Any]:
        """
        ...
        if metadata.type == "data":
            response = {"message": "Data uploaded successfully", **base_response}
            ...
            response = {"message": "Optimization metadata uploaded successfully", **base_response}
